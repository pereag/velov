...
class Maps {
  constructor(id, x, y, zoomMap){
      ...
      this.marker = [];
      this.response = false;
      this.reservation = new Reservation(document.getElementById("reservation-button"),
                                         document.getElementById("station-name"), document.getElementById("station-address"),
                                         document.getElementById("station-status"), document.getElementById("velov-number"),
                                         document.getElementById("map-info-station"), document.getElementById("error-info-station"),
                                         document.getElementById("map-reservation"), document.getElementById("delete"),
                                         document.getElementById("send"), document.getElementById("reservation-firstName"),
                                         document.getElementById("reservation-name"), document.getElementById("error-reservation"),
                                         document.getElementById("signature"), document.getElementById("form-reservation"),
                                         document.getElementById("confirmation-message"), document.getElementById("reservation-box-text"),
                                         document.getElementById("reservation-box-timer"), document.getElementById("reservation-box"), 1200000);
      this.mapStyle = [
          {
  ...
  initMap(){
      this.map = new google.maps.Map(
          this.id, {zoom: this.zoomMap, center: this.location,
                    styles: this.mapStyle, mapTypeControl: false,
    ...
    request.onreadystatechange = function() {
      ...
      // On crée une boucle pour crée les objets Marqueurs et Stations
        for (let i = 0; i < self.response.length; i++) {
          let station = new Station(self.response[i], document.getElementById("station-name"), document.getElementById("station-address"),document.getElementById("station-status"),
                                    document.getElementById("velov-number"));
